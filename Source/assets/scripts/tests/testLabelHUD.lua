...
font = JLI.World.getInstance():getWorldHUD():createFont("sans-ch", "fonts/DroidSansJapanese.ttf")
if (font == -1) then
    print("Could not add font italic.\n");
else
	print("yes " .. font)
...
font = JLI.World.getInstance():getWorldHUD():createFont("sans-ko", "fonts/malgun.ttf")
if (font == -1) then
    print("Could not add font italic.\n");
else
	print("yes " .. font)
...
local labelHUD = JLI.LabelHUD.create()
material:getDiffuse():loadGPU(labelHUD);
local imageWidth = 2048
local imageHeight = 2048

labelHUD:setFontFace("sans")
labelHUD:setText(JLI.LOCALIZED_TEXT("KEY"))
