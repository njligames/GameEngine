...
local icons = JLI.World.getInstance():getWorldHUD():createFont("icons", "fonts/entypo.ttf")
if (icons == -1) then
    print("Could not add font icons.\n");
end

local sliderHUD = JLI.SliderHUD.create()
material:getDiffuse():loadGPU(sliderHUD);
...
local v = sliderHUD:getValue()
function MyWorldStateUpdate(timeStep)
	sliderHUD:setValue(v)
	print (increment .. ' ' .. v)

	if sliderHUD:getValue() < 1 and sliderHUD:getValue() > 0 then
		v = v + increment
	else
		increment = -increment
		v = v + increment
