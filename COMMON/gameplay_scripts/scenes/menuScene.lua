...
local enter = function(self, scene)
    ...
    tbl[_menuSceneStateNames.menu] = 
    {
        ...
        -- {root = "STORE", name = "STORE", fun = function()
        --     pushSceneState(_menuSceneStateNames.store)
        -- end, toggle = false, soundPath = _SOUNDPATHS.interface.effects.select}, 
        -- {root = "HIGHSCORES", name = "HIGHSCORES", fun = function()
        --     pushSceneState(_menuSceneStateNames.highScores)
        -- end, toggle = false, soundPath = _SOUNDPATHS.interface.effects.select}, 
        -- {root = "SETTINGS", name = "SETTINGS", fun = function()
        --     pushSceneState(_menuSceneStateNames.settings)
        -- end, toggle = false, soundPath = _SOUNDPATHS.interface.effects.select}, 
        -- {root = "CHARACTERS", name = "CHARACTERS", fun = function()
    ...
    if _menuSceneStateNames.menu == stateName then
        ...
        -- print(deviceName,_menuNodePositions[stateName][name][deviceName], _menuNodePositions[stateName][name], _menuNodePositions[stateName])
        -- local x = _menuNodePositions[stateName][name][deviceName].x
        -- local y = _menuNodePositions[stateName][name][deviceName].y
...
local update = function(self, scene, timeStep)
    ...
    -- if stateName == _menuSceneStateNames.menu then
    --     if not self.worldNode:isTitleSongPlaying() then
    --         self.worldNode:playMenuSong()
    --     end
    -- end
