...
local createConstraint = function(self)
  ...
  -- self:getNode():getPhysicsBody():
--hasPhysicsConstraint
  

  local birdNode = self:getNode()
  local dogNode = self:getDog():getNode()

  -- if not birdNode:getPhysicsBody():hasPhysicsConstraint() then
  --   if not dogNode:getPhysicsBody():hasPhysicsConstraint() then
...
local update = function(self, timeStep)
  if not self:isPaused() then
    ...
    -- self:getNode():setRotation(njli.btQuaternion(bullet.btVector3(1,0,0), 0))
